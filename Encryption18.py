...
def ScramblingDecode(phrase3):
        ...
        for i in range(0,int(len(phrasepart1))):
            ...
            #If the phrase has an even number of characters, adds the last charaC:/Users/Pa Cyber/Documents/Encryption.txtcter.  Otherwise, it causes an error and stops.
            try:
...
while True:
    ...
    while True:
          ...
          if SystemKey.lower()=="y":
            RSACreator=input("\nGenerate a new public/private key, or use an old one? Y/N ")
            if RSACreator.lower()!="n":
              while True:
                try:
              ...
              while True:
                try:
                  ...
                  if purpose.lower()=="encrypt":
                      EncryptionNumber1=int(input("\nPlease input the public/private key to encrypt/decrypt the program. "))
                  else:
                      EncryptionNumber2=int(input("Please input the public/private key to encrypt/decrypt the program. "))
                  break
                except:
                  print("Please check your input.")
          else:
